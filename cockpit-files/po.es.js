cockpit.locale({
 "": {
  "plural-forms": (n) => n != 1,
  "language": "es",
  "language-direction": "ltr"
 },
 "\"$0\" exists, not overwriting with paste.": [
  null,
  "\"$0\" existe, no se permite pegar para sobreescribirlo."
 ],
 "$0 files": [
  null,
  "$0 archivos"
 ],
 "$0 files selected": [
  null,
  "$0 archivos seleccionados"
 ],
 "$0 item is hidden": [
  null,
  "$0 elemento está oculto",
  "$0 elementos están ocultos"
 ],
 "$0 permissions": [
  null,
  "$0 permisos"
 ],
 "(original owner)": [
  null,
  "(propietario original)"
 ],
 "A file with the same name already exists in \"$0\". Replacing it will overwrite its content.": [
  null,
  "Un archivo con el mismo nombre ya existe en \"$0\". Reemplazar sobreescribirá su contenido."
 ],
 "A-Z": [
  null,
  "A-Z"
 ],
 "Absolute": [
  null,
  "Absoluto"
 ],
 "Absolute symlinks are ideal when referring to files or directories that won't move, such as system files.": [
  null,
  "Enlaces simbólicos absolutos son ideales cuando se refieren a archivos o directorios que no se muevan, tales como archivos del sistema."
 ],
 "Absolute vs. Relative": [
  null,
  "Absoluto vs. Relativo"
 ],
 "Access": [
  null,
  "Acceso"
 ],
 "Activate selected item, enter directory": [
  null,
  "Activa el elemento seleccionado, entrar al directorio"
 ],
 "Add to bookmarks": [
  null,
  "Agregar a marcadores"
 ],
 "Apply this action to all conflicting files": [
  null,
  "Aplicar esta acción para todos los conflictos"
 ],
 "Block device": [
  null,
  "Dispositivo de bloque"
 ],
 "Bookmarks": [
  null,
  "Marcadores"
 ],
 "Cancel": [
  null,
  "Cancelar"
 ],
 "Cancel upload of $0": [
  null,
  "Cancelar la subida de $0"
 ],
 "Cancelled": [
  null,
  "Cancelado"
 ],
 "Cancelled upload of $0": [
  null,
  "Subida de $0 cancelada"
 ],
 "Cannot create file in current directory": [
  null,
  "No se puede crear el archivo en el directorio actual"
 ],
 "Cannot drop files, another upload is already in progress": [
  null,
  "No se pueden soltar archivos, ya hay una carga en progreso"
 ],
 "Change": [
  null,
  "Cambiar"
 ],
 "Change permissions": [
  null,
  "Modificar permisos"
 ],
 "Change permissions for enclosed files": [
  null,
  "Modificar permisos para los archivos incluidos"
 ],
 "Character device": [
  null,
  "Dispositivo de carácter"
 ],
 "Clear filter": [
  null,
  "Borrar filtro"
 ],
 "Close": [
  null,
  "Cerrar"
 ],
 "Cockpit Files": [
  null,
  "Cockpit Files"
 ],
 "Collapse": [
  null,
  "Colapsar"
 ],
 "Copy": [
  null,
  "Copiar"
 ],
 "Copy selected file or directory": [
  null,
  "Copiar archivo o directorio seleccionado"
 ],
 "Create": [
  null,
  "Crear"
 ],
 "Create directory": [
  null,
  "Crear directorio"
 ],
 "Create file": [
  null,
  "Crear archivo"
 ],
 "Create link": [
  null,
  "Crear enlace"
 ],
 "Create link to $0": [
  null,
  "Crear enlace a $0"
 ],
 "Create new directory": [
  null,
  "Crear nuevo directorio"
 ],
 "Delete": [
  null,
  "Eliminar"
 ],
 "Delete $0 items?": [
  null,
  "¿Eliminar $0 elementos?"
 ],
 "Delete $0?": [
  null,
  "¿Eliminar $0?"
 ],
 "Delete directory $0?": [
  null,
  "¿Eliminar directorio $0?"
 ],
 "Delete file $0?": [
  null,
  "¿Eliminar archivo $0?"
 ],
 "Delete link $0?": [
  null,
  "¿Eliminar enlace $0?"
 ],
 "Directory": [
  null,
  "Directorio"
 ],
 "Directory contains $0 directories, $1 files": [
  null,
  "El directorio contiene $0 directorios, $1 archivos"
 ],
 "Directory contains $0 directories, $1 files, $2 hidden": [
  null,
  "El directorio contiene $0 directorios, $1 archivos, $2 ocultos"
 ],
 "Directory is empty": [
  null,
  "El directorio está vacio"
 ],
 "Directory name": [
  null,
  "Nombre del directorio"
 ],
 "Directory name cannot be empty": [
  null,
  "El nombre del directorio no puede estar vacío"
 ],
 "Directory name cannot include a /": [
  null,
  "El nombre del directorio no puede incluir /"
 ],
 "Directory name too long": [
  null,
  "El nombre del directorio es demasiado largo"
 ],
 "Directory owner": [
  null,
  "Propietario del directorio"
 ],
 "Directory with the same name exists": [
  null,
  "Ya existe un directorio con el mismo nombre"
 ],
 "Display as a grid": [
  null,
  "Mostrar como una cuadrícula"
 ],
 "Display as a list": [
  null,
  "Mostrar como una lista"
 ],
 "Download": [
  null,
  "Descarga"
 ],
 "Drop files to upload": [
  null,
  "Suelta archivos para subir"
 ],
 "Edit $0": [
  null,
  "Editar $0"
 ],
 "Edit path": [
  null,
  "Editar ruta"
 ],
 "Edit permissions": [
  null,
  "Editar permisos"
 ],
 "Editing": [
  null,
  "Modificando"
 ],
 "Execute-only": [
  null,
  "Ejecutar solamente"
 ],
 "Failed": [
  null,
  "Falló"
 ],
 "File exists": [
  null,
  "El archivo existe"
 ],
 "File name": [
  null,
  "Nombre de archivo"
 ],
 "File uploaded": [
  null,
  "Archivo subido"
 ],
 "Filename is the same as original name": [
  null,
  "El nombre de archivo es el mismo que el nombre original"
 ],
 "Files being pasted have a different owner. By default, ownership will be changed to match the destination directory.": [
  null,
  "Archivos siendo pegados tienen un propietario diferente. Por defecto, la propiedad será modificada para coincidir con el directorio del destino."
 ],
 "Files uploaded": [
  null,
  "Archivos subidos"
 ],
 "Filesystem": [
  null,
  "Sistema de archivos"
 ],
 "Filter directory": [
  null,
  "Filtrar directorio"
 ],
 "First modified": [
  null,
  "Modificado por primera vez"
 ],
 "Force delete $0": [
  null,
  "Forzar borrado $0"
 ],
 "Force delete $0 items": [
  null,
  "Borrado forzado $0 elementos"
 ],
 "Force delete directory $0?": [
  null,
  "¿Borrado forsozo de directorio $0?"
 ],
 "Force delete file $0?": [
  null,
  "¿Borrado forzoso de archivo $0?"
 ],
 "Force delete link $0?": [
  null,
  "¿Borrado forzoso de enlace $0?"
 ],
 "Go back": [
  null,
  "Regresar"
 ],
 "Go forward": [
  null,
  "Avanzar"
 ],
 "Go up a directory": [
  null,
  "Subir un directorio"
 ],
 "Group": [
  null,
  "Grupo"
 ],
 "Group access": [
  null,
  "Acceso de grupo"
 ],
 "Hide hidden items": [
  null,
  "Ocultar elementos ocultos"
 ],
 "Home": [
  null,
  "Inicio"
 ],
 "Ignore": [
  null,
  "Ignorar"
 ],
 "Keep original": [
  null,
  "Mantener original"
 ],
 "Keyboard shortcuts": [
  null,
  "Atajos de teclado"
 ],
 "Largest size": [
  null,
  "Tamaño más grande"
 ],
 "Last modified": [
  null,
  "Última modificación"
 ],
 "Least permissive": [
  null,
  "Menos permisivo"
 ],
 "Missing type": [
  null,
  "Tipo faltante"
 ],
 "Modified": [
  null,
  "Modificado"
 ],
 "Most permissive": [
  null,
  "Más permisivo"
 ],
 "Name": [
  null,
  "Nombre"
 ],
 "Name cannot be empty": [
  null,
  "El nombre no puede estar vacío"
 ],
 "Name cannot include a /": [
  null,
  "El nombre no puede contener /"
 ],
 "Name too long": [
  null,
  "El nombre es demasiado largo"
 ],
 "Named pipe": [
  null,
  "Conducto nombrado"
 ],
 "Navigation": [
  null,
  "Navegación"
 ],
 "New file": [
  null,
  "Nuevo archivo"
 ],
 "New filename": [
  null,
  "Nuevo nombre de archivo"
 ],
 "New name": [
  null,
  "Nuevo nombre"
 ],
 "New owner": [
  null,
  "Nuevo propietario"
 ],
 "No access": [
  null,
  "No hay acceso"
 ],
 "No matching results": [
  null,
  "No se encontraron resultados"
 ],
 "None": [
  null,
  "Ninguno"
 ],
 "Open in terminal": [
  null,
  "Abrir en terminal"
 ],
 "Open text file": [
  null,
  "Abrir archivo de texto"
 ],
 "Original file on server": [
  null,
  "Archivo original en el servidor"
 ],
 "Others": [
  null,
  "Otros"
 ],
 "Others access": [
  null,
  "Otros accesos"
 ],
 "Overwrite": [
  null,
  "Sobrescribir"
 ],
 "Owner": [
  null,
  "Propietario"
 ],
 "Owner access": [
  null,
  "Acceso de propietario"
 ],
 "Owner ascending": [
  null,
  "Propetario ascendente"
 ],
 "Owner descending": [
  null,
  "Propetario descendiente"
 ],
 "Ownership": [
  null,
  "Propiedad"
 ],
 "Paste": [
  null,
  "Pegar"
 ],
 "Paste as owner": [
  null,
  "Pegar como propietario"
 ],
 "Paste file or directory": [
  null,
  "Pegar archivo o directorio"
 ],
 "Pasting failed": [
  null,
  "Falló el pegado"
 ],
 "Permission denied": [
  null,
  "Permiso denegado"
 ],
 "Permissions": [
  null,
  "Permisos"
 ],
 "Permissions for $0 files": [
  null,
  "Permisos para $0 archivos"
 ],
 "Read and execute": [
  null,
  "Lectura y ejecución"
 ],
 "Read and write": [
  null,
  "Lectura y escritura"
 ],
 "Read, write, and execute": [
  null,
  "Lectura, escritura y ejecución"
 ],
 "Read-only": [
  null,
  "Solo lectura"
 ],
 "Regular file": [
  null,
  "Archivo regular"
 ],
 "Relative": [
  null,
  "Relativo"
 ],
 "Relative symlinks are useful when both a symlink and target might move at the same time, such as when renaming or moving a parent directory.": [
  null,
  "Enlaces simbólicos relativos son útiles cuando ambos enlace y destino muevan a la vez, tales como cuando renombren o muevan un directorio antecesor."
 ],
 "Reload": [
  null,
  "Recargar"
 ],
 "Remove from bookmarks": [
  null,
  "Quitar de marcadores"
 ],
 "Rename": [
  null,
  "Renombrar"
 ],
 "Rename $0?": [
  null,
  "¿Renombrar $0?"
 ],
 "Rename selected file or directory": [
  null,
  "Renombrar archivo o directorio seleccionado"
 ],
 "Replace": [
  null,
  "Reemplazar"
 ],
 "Replace file $0?": [
  null,
  "¿Reemplazar archivo $0?"
 ],
 "Save": [
  null,
  "Guardar"
 ],
 "Security context": [
  null,
  "Contexto de seguridad"
 ],
 "Select all": [
  null,
  "Seleccionar todo"
 ],
 "Set executable as program": [
  null,
  "Establecer como programa ejecutable"
 ],
 "Show all files": [
  null,
  "Mostrar todos los archivos"
 ],
 "Show hidden items": [
  null,
  "Mostrar elementos ocultos"
 ],
 "Show keyboard shortcuts": [
  null,
  "Mostrar atajos de teclado"
 ],
 "Size": [
  null,
  "Tamaño"
 ],
 "Smallest size": [
  null,
  "Tamaño más pequeño"
 ],
 "Socket": [
  null,
  "Zócalo"
 ],
 "Sort": [
  null,
  "Ordenar"
 ],
 "Symbolic link": [
  null,
  "Enlace simbólico"
 ],
 "Symlink name": [
  null,
  "Nombre de enlace simbólico"
 ],
 "Target": [
  null,
  "Destino"
 ],
 "The existing file changed unexpectedly": [
  null,
  "El archivo existente cambió de manera inesperada"
 ],
 "The file has been removed on disk": [
  null,
  "El archivo ha sido eliminado del disco"
 ],
 "The file has changed on disk": [
  null,
  "El archivo ha cambiado en el disco"
 ],
 "Type": [
  null,
  "Tipo"
 ],
 "Unable to create bookmark directory": [
  null,
  "No es posible crear el marcador de directorio"
 ],
 "Unable to move uploaded file to final destination": [
  null,
  "No se puede mover el archivo subido a su destino final"
 ],
 "Unable to save bookmark file": [
  null,
  "No es posible guardar el archivo de marcador"
 ],
 "Upload": [
  null,
  "Subir"
 ],
 "Upload $0% completed": [
  null,
  "Carga del $0% completado"
 ],
 "Upload error": [
  null,
  "Error de subida"
 ],
 "Uploaded as $0, $1": [
  null,
  "Subido como $0, $1"
 ],
 "Uploads": [
  null,
  "Subidas"
 ],
 "User": [
  null,
  "Usuario"
 ],
 "View $0": [
  null,
  "Ver $0"
 ],
 "Write and execute": [
  null,
  "Escritura y ejecución"
 ],
 "Write-only": [
  null,
  "Solo escritura"
 ],
 "Z-A": [
  null,
  "Z-A"
 ],
 "keep original owners": [
  null,
  "mantener propietarios originales"
 ],
 "link to $0": [
  null,
  "enlazar a $0"
 ]
});
